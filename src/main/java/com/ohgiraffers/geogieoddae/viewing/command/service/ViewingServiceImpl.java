...
public class ViewingServiceImpl implements ViewingService {
    ...
    public void createViewing(ViewingDto dto) {
        ...
        RestaurantEntity restaurant = restaurantRepository.findById(dto.getRestaurantId())
                .orElseThrow(() -> new ViewingRegisterException(ErrorCode.FAILED_VIEWING_REGISTER));
        ...
        if (dto.getTeamId() != null && !dto.getTeamId().isEmpty()) {
            ...
            if (teams.isEmpty()) {
                throw new ViewingRegisterException(ErrorCode.FAILED_VIEWING_REGISTER);
            }
            if (teams.size() > 2) {
                throw new ViewingRegisterException(ErrorCode.FAILED_VIEWING_REGISTER);
            }
    ...
    public void updateViewing(ViewingDto dto) {
        ...
        if (viewingUserRepository.existsByViewing_ViewingCode(dto.getViewingCode())) {
            throw new ViewingUpdateException(ErrorCode.FAILED_VIEWING_UPDATE);
        }
    ...
    public void deleteViewing(Long viewingCode) {
        ViewingEntity viewing = viewingRepository.findById(viewingCode)
                .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 관람입니다."));
        if(viewingUserRepository.existsByViewing_ViewingCode(viewingCode)){
            throw new ViewingDeleteException(ErrorCode.FAILED_VIEWING_DELETE);
        }
